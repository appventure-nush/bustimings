...
<html style='height:100%;'>
    ...
    <body style='height:97%;'>
        ...
        function reorder(&$a) {
            ...
            if ($diff->invert == 1) {
                $a["NextBus"]["EstimatedArrival"] = $a["SubsequentBus"]["EstimatedArrival"];
                $a["SubsequentBus"]["EstimatedArrival"] = $a["SubsequentBus3"]["EstimatedArrival"];
                $a["SubsequentBus3"]["EstimatedArrival"] = "";
                $a["NextBus"]["Load"] = $a["SubsequentBus"]["Load"];
                $a["SubsequentBus"]["Load"] = $a["SubsequentBus3"]["Load"];
                $a["SubsequentBus3"]["Load"] = "";
            }
        }
        ...
        for ($i = 0; $i < count($busstops); $i++) {
            ...
            for ($ii = 0; $ii < count($out['Services']); $ii++) {
                if (isset($_GET['show']) || ($out['Services'][$ii]["ServiceNo"] != '97e' && $out['Services'][$ii]["ServiceNo"] != '963R')) {
                    ...
                    while ($out['Services'][$ii]["NextBus"]["EstimatedArrival"]!=""&&isset($_GET['cut']) && $diff->i < $_GET['cut'] && $i > 1) {
                        $out['Services'][$ii]["NextBus"]["EstimatedArrival"] = $out['Services'][$ii]["SubsequentBus"]["EstimatedArrival"];
                        $out['Services'][$ii]["SubsequentBus"]["EstimatedArrival"] = $out['Services'][$ii]["SubsequentBus3"]["EstimatedArrival"];
                        $out['Services'][$ii]["SubsequentBus3"]["EstimatedArrival"] = "";
                        $out['Services'][$ii]["NextBus"]["Load"] = $out['Services'][$ii]["SubsequentBus"]["Load"];
                        $out['Services'][$ii]["SubsequentBus"]["Load"] = $out['Services'][$ii]["SubsequentBus3"]["Load"];
                        $out['Services'][$ii]["SubsequentBus3"]["Load"] = "";
                        $diff = $now->diff(new DateTime($out['Services'][$ii]["NextBus"]["EstimatedArrival"]));
                    }
            ...
            for ($ii = 0; $ii < count($out['Services']); $ii++) {
                if (isset($_GET['show']) || ($out['Services'][$ii]["ServiceNo"] != '97e' && $out['Services'][$ii]["ServiceNo"] != '963R')) {
                    echo "<td class='time' style='border-width: 0px 1px 0px 0px;background-color:" . getcolor($out['Services'][$ii]["SubsequentBus"]["Load"]) . "'>" . gettime($out['Services'][$ii]["SubsequentBus"]["EstimatedArrival"]) . "</td>";
                }
            }
            echo "</tr><tr>";
            for ($ii = 0; $ii < count($out['Services']); $ii++) {
                if (isset($_GET['show']) || ($out['Services'][$ii]["ServiceNo"] != '97e' && $out['Services'][$ii]["ServiceNo"] != '963R')) {
                    ...
                    echo "<td class='time' style='border-width: 0px 1px 1px 0px;background-color:" . getcolor($out['Services'][$ii]["SubsequentBus3"]["Load"]) . "'>" . gettime($out['Services'][$ii]["SubsequentBus3"]["EstimatedArrival"]) . "</td>";
                }
            }
            echo "</tr>";
            // echo var_dump($out);
